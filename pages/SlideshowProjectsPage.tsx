...
interface ProjectListItemProps {
  ...
  activeSphereId: string; // New prop
}
  ...
  useEffect(() => {
    const countImages = async () => {
      if (project.imageIds && project.imageIds.length > 0) {
        ...
        const validImages = resolvedImages.filter(img => img && img.dataUrl && img.sphereId === activeSphereId);
...
export const SlideshowProjectsPage: React.FC<SlideshowProjectsPageProps> = ({ onNavigate, currentUser, activeSphere }) => {
  ...
  const handleGeneratePdfForProject = async (projectId: string) => {
    ...
    try {
      ...
      const imagesForProjectPromises = projectToExport.imageIds.map(id => getImageById(id));
      const resolvedImages = await Promise.all(imagesForProjectPromises);
      
      const imagesForProject = resolvedImages
        .filter(img => img && img.dataUrl && img.sphereId === activeSphere.id) as ImageRecord[]; 
          ...
          <div className="bg-slate-50 dark:bg-slate-700/80 p-6 sm:p-8 rounded-xl shadow-lg mb-10 border border-border-color dark:border-slate-600">
            ...
            {availableImagesForSelection.length === 0 ? (
                <div className="bg-yellow-50 dark:bg-yellow-500/20 border border-yellow-300 dark:border-yellow-500 text-yellow-700 dark:text-yellow-300 px-4 py-3 rounded-lg my-4">
                    <p className="font-semibold">Inga bilder tillgängliga i denna sfär!</p>
                    <p>Lägg till bilder i Bildbanken för sfären "{activeSphere.name}" för att kunna skapa projekt.</p>
                </div>
            ) : (
