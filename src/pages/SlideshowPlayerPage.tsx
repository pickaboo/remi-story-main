    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') {
        if (document.fullscreenElement) {
          ...
          handleNavigate('/slideshow-projects'); 
        }
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isLoading, error, images.length, goToNextImage, goToPrevImage, toggleFullScreen, handleNavigate]);
  ...
  if (error || !project || images.length === 0) {
    return (
      <div className="fixed inset-0 bg-dark-bg text-white flex flex-col items-center justify-center p-8 z-[100]">
        ...
        <button 
            onClick={() => handleNavigate('/slideshow-projects')} 
